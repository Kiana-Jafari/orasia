...
function ContactPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      console.error(err);
      // اینجا می‌تونی یه state خطا هم اضافه کنی برای نمایش به کاربر
    }
  }

  const fields = [
    { id: "fullName", label: t.contact.fields.fullName, type: "text", required: true, autoComplete: "name" },
    { id: "companyName", label: t.contact.fields.companyName, type: "text", required: false, autoComplete: "organization" },
    { id: "email", label: t.contact.fields.email, type: "email", required: true, autoComplete: "email" },
    { id: "phone", label: t.contact.fields.phone, type: "tel", required: false, autoComplete: "tel" },
    { id: "subject", label: t.contact.fields.subject, type: "text", required: true, autoComplete: "off" },
  ] as const;

  const details = [
    { label: t.contact.info.companyName, value: t.company.name },
    { label: t.contact.info.address, value: t.company.address },
    { label: t.contact.info.phone, value: companyContact.phone, href: `tel:${companyContact.phone}` },
    { label: t.contact.info.email, value: companyContact.email, href: `mailto:${companyContact.email}` },
    { label: t.contact.info.whatsapp, value: companyContact.whatsapp },
    { label: t.contact.info.hours, value: t.company.hours },
    { label: t.contact.info.website, value: companyContact.website },
  ];
      ...
      <Section>
        <div className="grid gap-14 md:grid-cols-[0.85fr_1.15fr] md:gap-20">
          <div>
            ...
            <p className="mt-6 text-xs text-muted-foreground">{t.contact.detailsNote}</p>
          </div>
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="mt-8 grid gap-6 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 flex flex-wrap items-center gap-4">
                <Button type="submit">{t.actions.sendInquiry}</Button>
                {sent ? (
                  <p role="status" className="reveal flex items-center gap-2 text-sm text-muted-foreground">
                    <svg
                      aria-hidden="true"
                      viewBox="0 0 24 24"
